    ...
    public class EZRectTransformAnimationEditor : EZAnimationEditor
    {
        ...
        protected override void DrawPhaseListElement(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            if (m_DrivePosition.boolValue)
            {
                ...
                EditorGUI.PropertyField(new Rect(x1, rect.y, width - margin, height), startValue.FindPropertyRelative("m_AnchoredPosition"), GUIContent.none);
                EditorGUI.PropertyField(new Rect(x2, rect.y, width - margin, height), endValue.FindPropertyRelative("m_AnchoredPosition"), GUIContent.none);
                rect.y += rect.height;
            }
            if (m_DriveSize.boolValue)
            {
                GUI.backgroundColor = sizeColor;
                EditorGUI.PropertyField(new Rect(x1, rect.y, width - margin, height), startValue.FindPropertyRelative("m_SizeDelta"), GUIContent.none);
                EditorGUI.PropertyField(new Rect(x2, rect.y, width - margin, height), endValue.FindPropertyRelative("m_SizeDelta"), GUIContent.none);
                rect.y += rect.height;
            }
