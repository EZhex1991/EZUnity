...
namespace EZUnity.Example
{
    ...
    public class Hotfix : MonoBehaviour
    {
        ...
        public void Fix()
        {
            ...
            luaEnv.DoString(@"
                xlua.hotfix(CS.EZUnity.Example.Hotfix, 'Update', function(self)   -- private方法，直接fix
        ...
        public void FixClass()
        {
            ...
            luaEnv.DoString(@"
                xlua.private_accessible(CS.EZUnity.Example.Hotfix)   -- 获取private字段的访问权限
                xlua.hotfix(CS.EZUnity.Example.Hotfix, {  -- 直接Fix整个Class的写法
                ...
                NewCor2_FixClear = function(self) -- 原Cor2_FixClear返回值错误，这里实际上是lua端的新增函数而不是修复函数
                    return require('xlua.util').cs_generator(function()
                        ...
                        self.luaEnv:DoString('xlua.hotfix(CS.EZUnity.Example.Hotfix, { Update = false, FixClear = false })') -- 清空Hotfix
                        print('Cleared')
                    end)
                end
            ");
