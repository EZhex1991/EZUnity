...
{
    [CreateAssetMenu(fileName = "EZWaveTextureGenerator", menuName = "EZUnity/EZWaveTextureGenerator", order = (int)EZAssetMenuOrder.EZWaveTextureGenerator)]
            ...
            if (antialiasing == Antialiasing.X2)
            {
                for (int x = 0; x < texture.width; x++)
                {
                    ...
                    for (int y = 0; y < texture.height; y++)
                    {
                        ...
                        Color color = Color.Lerp(colorUp, colorDown, average);
